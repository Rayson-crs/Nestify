...
import { isImageClip, normalizeImageClip } from './image-clip.ts'

const IMAGE_EXTENSIONS: Record<'jpg' | 'png' | 'webp', string> = {
  jpg: '.jpg',
  png: '.png',
  webp: '.webp',
}
...
export async function buildMediaMergePlan(input: MediaMergePlanInput): Promise<MediaMergePlan> {
  ...
  if (input.items.length < 1) throw new Error('合并至少需要一个媒体文件')
  if (input.kind === 'image' && input.items.length < 2) {
    throw new Error('图片合并至少需要两个图片文件')
  }
  ...
  const normalizedOutputName = `${parsedName.name}${extension}`
  const outputPath = join(outputDirectory, normalizedOutputName)
  if (items.some((item) => samePath(item.path, outputPath))) {
  ...
  if (imageSettings) {
    try {
      imageSummary = await buildImageSummary(items, imageSettings)
    } catch {
...
async function buildImageSummary(
  ...
  for (const item of items) {
    ...
    const width = metadata.orientation != null && metadata.orientation >= 5 ? metadata.height : metadata.width
    const height = metadata.orientation != null && metadata.orientation >= 5 ? metadata.width : metadata.height
