...
Path('apps/desktop/src/app/useAppWorkspace.ts').write_text(r'''import { useCallback, useEffect, useState } from 'react'
...
export function useAppWorkspace(): AppViewModel {
  ...
  const plans = usePlans({
    ...
    selectedHitParent: search.selectedHit?.parent ?? null,
    query: search.query,
    tab,
    setTab,
    setError,
...
''', encoding='utf-8')
print('useAppWorkspace.ts written', Path('apps/desktop/src/app/useAppWorkspace.ts').stat().st_size)
