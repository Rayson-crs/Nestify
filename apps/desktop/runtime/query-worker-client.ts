...
export class QueryWorkerClient {
  ...
  cancel(kind: PendingRequest['kind'] = 'search'): void {
  ...
  async close(): Promise<void> {
    ...
    if (workers.length) this.onLog?.('worker.close', { type: 'query', count: workers.length })
  ...
  private ensureWorker(kind: PendingRequest['kind']): Worker {
    ...
    this.onLog?.('worker.start', { type: 'query', kind })
    ...
    worker.on('message', (message: QueryWorkerMessage) => {
      if (message.type === 'ready') {
        resolveReady()
        this.onLog?.('worker.ready', {
          type: 'query',
          kind,
          startupMs: message.startupMs,
          pragmaMs: message.pragmaMs,
        })
        return
      }
    ...
    worker.on('exit', (code) => {
      if (this.getWorker(kind) === worker) {
        this.onLog?.('worker.exit', { type: 'query', kind, code, expected: false }, 'error')
        this.setWorker(kind, null)
        this.rejectPending(new Error(`query worker exited with code ${code}`), kind)
  ...
  private handleWorkerFailure(kind: PendingRequest['kind'], worker: Worker, error: Error): void {
    this.onLog?.('worker.error', { type: 'query', kind, error }, 'error')
    if (this.getWorker(kind) === worker) {
      this.setWorker(kind, null)
      this.rejectReady(kind, error)
