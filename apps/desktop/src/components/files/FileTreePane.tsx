        ...
        {rootMode ? (
          ...
          <div className="flex min-w-0 flex-1 items-center gap-1 overflow-x-auto overflow-y-hidden whitespace-nowrap [scrollbar-width:thin]">
            {crumbs.map((crumb, index) => {
              ...
              return (
                <div key={crumb.path} className="flex min-w-0 max-w-[min(14rem,32vw)] shrink-0 items-center gap-1">
                  {index > 0 ? <ChevronRight className="h-3.5 w-3.5 shrink-0 text-muted-foreground" /> : null}
                  <Button
                    variant={last ? 'secondary' : 'ghost'}
                    size="sm"
                    className="min-w-0 max-w-full justify-start overflow-hidden px-2"
                    title={crumb.path}
