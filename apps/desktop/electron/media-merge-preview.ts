...
}): Promise<{ src: string; width: number; height: number; error: string | null }> {
  try {
    ...
    return { src: mediaPreviewUrl(filePath), width, height, error: null }
  } catch (error) {
    return imageError(errorText(error))
  }
}
...
async function existingProxy(key: string): Promise<ProxyResult | null> {
  ...
  try {
    ...
    if (existing.isFile() && existing.size > 0) {
      return { src: mediaPreviewUrl(outputPath), status: 'ready', error: null }
    }
  } catch {
    return null
  }
...
function imageError(error: string): { src: string; width: number; height: number; error: string } {
