...
> 当前状态（2026-09-22 / v1.9.0）：产品名是 Nestify。桌面工作台五个入口是文件、整理、改名、重复、任务。设置里有「关于」。版本只写在根目录 `package.json`，启动和打包同步到桌面应用。Windows x64 portable 产物是 `apps/desktop/release/Nestify-v1.9.0.exe`。历史过程保留在各篇正文，不以 2026-09-10 的目标冒充已交付。
