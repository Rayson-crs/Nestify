...
> 文档版本：v0.4
> 当前状态（2026-09-17 / v1.8.0）：P0-P3 基础实现已完成。schema 仍是 v7。Query / Writer Worker 已落地，Preview Worker 是本 TRD 之后补上的缩略图隔离。物理冷缓存、真实 Worker 并发、3M/10M、SMB 与 watcher overflow 尚未验收，P4 搜索索引决策待完成。下文保留 2026-09-10 的问题现场和基准记录。
...
2026-09-10 的现场：启动显示“未响应”，直接原因不是数据库打开或迁移，而是 Renderer 自动发起全库空搜索。空搜索对约 52 万条记录执行精确总数、文件/目录统计、类型分组和排序列表，查询当时通过 `DatabaseSync` 在 Electron Main 主线程同步执行，阻塞窗口事件循环。

v1.8.0 对照：启动已不再做全库空搜索；搜索和目录查询走 Query Worker；Writer Worker 负责增量写入。剩余缺口是扫描/规则/计划/重复分析仍可能走 Main，以及物理冷缓存、真实 Worker 并发、3M/10M、SMB、watcher overflow 尚未验收。P4 独立搜索引擎仍待决策。
...
索引并不能覆盖当时全部性能问题。搜索还受到查询起点、`library_entries` 关联方向、重复统计、临时排序、路径规范化函数以及主进程同步执行的影响。这些项里，启动空搜索和 Query Worker 隔离已经落地；主进程同步执行仍可能出现在扫描、规则、计划和重复分析路径。
...
第一阶段不立即替换 SQLite，也不立即删除三元组表。启动路径、SQL 和 Query/Writer 隔离已经按这个顺序落地；是否引入独立搜索引擎仍要等中文模糊搜索和冷缓存基准后再决定。
...
> 实施边界：P0、P1、P3 已落地，P2 已完成 Query Worker 和基础 Writer Worker 接入。v1.8.0 另外有 Preview Worker 和 Library-removal Worker。扫描/规则/计划/重复分析仍未全部移出 Main，P4 独立搜索引擎仍需基准测试后决策。下文的“目标进程模型”是最终架构，不代表当前代码已经全部完成。
...
P0 已完成：启动不再做全库空搜索，直接消除当时 12-28 秒级的启动阻塞。P1 查询计划和 P2 Query/Writer Worker 也已落地。当前剩余工作是扫描/规则/计划/重复分析继续移出 Main、冷缓存/3M/10M/SMB 验收，以及 P4 引擎决策。基准入口为 `npm run benchmark:search -- --rows 1000000 --iterations 1000`；只有在这些改造和压测后仍无法满足中文模糊搜索目标，才引入独立搜索引擎。
