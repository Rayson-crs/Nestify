...
Path('apps/desktop/src/app/useLibraries.ts').write_text(r'''import { useCallback, useEffect, useMemo, useState } from 'react'
import {
  ALL_LIBRARIES_ID,
  callNestify,
  getNestifyApi,
...
export function useLibraries(options: {
  ...
  setTreePath: (value: string | null) => void
  pendingTreePath: { current: string | null }
}) {
  ...
  const treeRootPath = useMemo(() => {
    ...
    const currentPath = options.treePath ?? ''
    return roots.find((root) => isWithinDirectory(currentPath, root)) ?? (allLibrariesSelected ? null : roots[0] ?? null)
  }, [allLibrariesSelected, libraries, options.treePath, selectedLibrary])
  ...
  useEffect(() => {
    setBusy(null)
    const current = libraries.find((library) => library.id === selectedLibraryId) ?? null
    const requestedPath = options.pendingTreePath.current
    options.pendingTreePath.current = null
    const requestedRoot =
      requestedPath && current
        ? current.roots.find((root) => isWithinDirectory(requestedPath, root))
        : undefined
    options.setTreePath(allLibrariesSelected ? null : requestedRoot ? requestedPath : current?.roots[0] ?? null)
  }, [allLibrariesSelected, libraries, options, selectedLibraryId])
  ...
  useEffect(() => {
    ...
    void callNestify((api) => api.scanProgress())
      ...
      .catch((err) => options.setError(errorMessage(err)))
  }, [ipcReady, options])
  ...
  const handleAddLibrary = async () => {
    ...
    try {
      ...
      options.setNotice(`已添加资料库 ${library.name}`)
    } catch (err) {
      options.setError(errorMessage(err))
    } finally {
      setBusy(null)
    }
  }
  ...
  const handleScanControl = async (action: 'pause' | 'resume' | 'cancel', onChanged?: () => Promise<void> | void) => {
    if (!scanJobId) return
    setBusy(`scan:${action}`)
    options.setError(null)
    try {
      ...
      options.setNotice(action === 'pause' ? '扫描已暂停' : action === 'resume' ? '扫描已恢复' : '正在取消扫描')
      await onChanged?.()
    } catch (err) {
      options.setError(errorMessage(err))
    } finally {
  ...
  const handleUpdateLibrary = async () => {
    ...
    if (!name) {
      options.setError('资料库名称不能为空')
      return
    }
    if (roots.length === 0) {
      options.setError('至少保留一个根目录')
      return
    }
    if (maxDepth != null && (!Number.isInteger(maxDepth) || maxDepth < 0)) {
      options.setError('最大深度必须是空或非负整数')
      return
    }
    ...
    try {
      ...
      options.setNotice(`已保存资料库 ${library.name}`)
    } catch (err) {
      options.setError(errorMessage(err))
    } finally {
      setBusy(null)
